...
def scan_page(
    ...
    text, _ = extract_text_and_links(html, url)
    hits: list[Hit] = []
    for word in words:
        if word_present(text, word):
            hits.append(Hit(word=word, url=url, scanned_at=scanned_at))
    return hits
...
def collect_urls_to_scan(seed_urls: Iterable[str], session: requests.Session) -> list[str]:
    ...
    for seed in seed_urls:
        ...
        if path.count("/") <= 1:
            try:
                html = fetch_html(url, session)
                _, links = extract_text_and_links(html, url)
...
def write_report(hits: list[Hit], reports_dir: Path, generated_at: datetime) -> Path:
    ...
    headers = ("слово", "ссылка на страницу", "дата время скана")
    ws.append(headers)
    for cell in ws[1]:
